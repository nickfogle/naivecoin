...
class HttpServer {
    constructor(node, blockchain, operator, miner) {
        ...
        this.app.use('/', express.static('lib/dashboard/build'));
        ...
        this.app.get('/blockchain/:addressId/heartbeat', (req, res) => {
            let addressId = req.params.addressId;

            try {
                let tx = blockchain.getHeartbeatTransactionsForAddress(addressId);
                res.status(200).send(tx);
            } catch (ex) {
                if (ex instanceof ArgumentError) throw new HTTPError(400, ex.message, { addressId }, ex);
                else throw ex;
            }
        });
        ...
        this.app.post('/operator/wallets/:walletId/heartbeat', (req, res) => {
            let walletId = req.params.walletId;
            let password = req.headers.password;
            ...
            try {
                ...
                let newTransaction = operator.createHeartbeatTransaction(walletId, req.body.fromAddress, req.body.data);
        ...
        this.app.get('/node/peers/refresh', (req, res) => {
            res.status(200).send(node.refreshFromPeers());
        });
