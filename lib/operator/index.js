...
class Operator {
    ...
    getHeartbeatTransactionsForWalletAddress(walletId, addressId) {
        let address = this.getAddressesForWallet(walletId, addressId);
        let txs = this.blockchain.getTransactionsForAddress(address);
        return txs.filter(tx => tx.type === 'heartbeat');
    }
    ...
    createHeartbeatTransaction(walletId, fromAddressId, data) {
        let utxo = this.blockchain.getUnspentTransactionsForAddress(fromAddressId);
        let wallet = this.getWalletById(walletId);
        ...
        let tx = new TransactionBuilder();
        tx.from(utxo);
        tx.to(fromAddressId, 0);
        tx.fee(10);
        tx.change(fromAddressId);
        tx.sign(secretKey);        
        tx.setData(data);
        tx.setType('heartbeat');
        return Transaction.fromJson(tx.build());
    }
}
